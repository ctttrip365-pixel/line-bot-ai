...
export interface ProposalWithMatch {
  ...
  // true สำหรับ booking ที่ยังไม่เคยถูกยืนยันจริง (ไม่มีแถวเลย หรือค้าง "proposed" ยังไม่ได้กดยืนยัน —
  // ไม่เกี่ยวกับวันที่รับ booking เข้ามาหรือเคยเสนอมาแล้วกี่รอบ) — ข้ามขั้น "รอกดยืนยัน" ไปเลย ยืนยัน
  // อัตโนมัติทันที (ยังแจ้ง Telegram แบบ FYI + ปุ่มเปลี่ยนคนขับไว้เผื่อแชมป์อยากสลับทีหลัง) ส่วน booking
  // ที่เคย confirmed แล้วแต่ถูกปลดกลับมา (needs_reassignment — เช่นจากการอนุมัติลา) ยังต้องผ่านขั้นกด
  // ยืนยันเหมือนเดิมเสมอ ไม่ auto-confirm ให้ (ตามที่แชมป์ยืนยัน 2026-09-23)
  autoConfirm?: boolean;
}
